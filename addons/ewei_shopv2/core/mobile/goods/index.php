...
class Index_EweiShopV2Page extends MobilePage 
{
	...
	public function get_list() 
	{
		global $_GPC;
		global $_W;
		$args = array('pagesize' => 100, 'page' => intval($_GPC['page']), 'isnew' => trim($_GPC['isnew']), 'ishot' => trim($_GPC['ishot']), 'isrecommand' => trim($_GPC['isrecommand']), 'isdiscount' => trim($_GPC['isdiscount']), 'istime' => trim($_GPC['istime']), 'issendfree' => trim($_GPC['issendfree']), 'keywords' => trim($_GPC['keywords']), 'cate' => trim($_GPC['cate']), 'order' => trim($_GPC['order']), 'by' => trim($_GPC['by']));
		$plugin_commission = p('commission');
	...
	private function _condition($args) 
	{
		...
		foreach ($goods['list'] as $key=>$value) {
				if($value['pcate']==1179  || $value['pcate']==1180){
					unset($goods['list'][$key]);
				}
		}
		$goods['list']=array_merge($goods['list']);
		
		show_json(1, array('list' => $goods['list'], 'total' => $goods['total'], 'pagesize' => $args['pagesize']));
	}
	
	private function _condition1($args) 
	{
		global $_GPC;
		$merch_plugin = p('merch');
		$merch_data = m('common')->getPluginset('merch');
		...
		foreach ($list as $key => $value) {
				if(intval($value['pcate'])>1178){
					unset($list[$key]);
				}
		}		
		$list=array_merge($list,$list);
		show_json(1, array('list' => $list, 'total' => $goods['total'], 'pagesize' => $args['pagesize']));
	}	
}
?>