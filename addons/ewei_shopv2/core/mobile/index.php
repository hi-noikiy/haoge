...
class Index_EweiShopV2Page extends MobilePage 
{
	public function main() 
	{
		...
		$category = m('shop')->getCategory()['parent'];	
		
		//var_dump($category);
			
		$wapset = m('common')->getSysset('shop');	
		$shareset = m('common')->getSysset('share');//followurl
		$notice= pdo_fetchall('select * from '.tablename('ewei_shop_notice').' where  status=1  order by  id  desc  LIMIT 0,10');
	...
	public function get_recommand() 
	{
		...
		foreach ($list as $key => $value) {
			$list[$key]['newlogo']=tomedia($list[$key]['newlogo']);
		}
		show_json(1, array('list' =>$list, 'pagesize' => $args['pagesize'], 'total' => $recommand['total'], 'page' => intval($_GPC['page'])));
	}
	public function get_new() 
	{
		global $_W;
		global $_GPC;
		pdo_update('ewei_shop_member',array('isview'=>1),array('openid'=>$_W['openid']));
		$args = array('page' => $_GPC['page'], 'pagesize' => 6, 'isnew' => 1, 'order' => 'displayorder desc,createtime desc', 'by' => '');
		$recommand = m('goods')->getList($args);
		$list=$recommand['list'];
		foreach ($list as $key => $value) {
			$list[$key]['newlogo']=tomedia($list[$key]['newlogo']);
