...
class Cart_EweiShopV2Page extends MobileLoginPage 
{
	...
	public function cartselect()
	{
		...
		$mainid=$_GPC['mainid'];
		$maintotal=$_GPC['maintotal'];
		$mainoptionid=$_GPC['mainoptionid'];
		$selectids=$_GPC['selectids'];
		...
		if($mainid==''){
			...
			$type=2;			
			pdo_update('ewei_shop_member_cart',array('selected'=>0));
		}
		
		//用户主商品
		$maingoods=pdo_fetchall("select *  from ".tablename('ewei_shop_goods')." where id=(".intval($mainid).")");		
		foreach ($maingoods as $key=>$value) {
			$maingoods[$key]['total']=$maintotal;
			$maingoods[$key]['optionid']=$mainoptionid;	
		}	
		...
		$selectgoods=pdo_fetchall("select *  from ".tablename('ewei_shop_goods')." where id in(".$selectids.")");
		foreach ($selectgoods as $key=>$value) {
				$selectgoods[$key]['total']=1;
		}
			
		$goods=array_merge($maingoods,$selectgoods);
		
		foreach ($goods as $value) {
			if($type==1){
				$data = array('uniacid' => $_W['uniacid'],'openid' => $_W['openid'], 'goodsid' => $value['id'], 'marketprice' => $value['marketprice'], 'total' => 1, 'selected' => 1,'createtime' => time());				
			}
			if($type==2){
				$data = array('uniacid' => $_W['uniacid'],'openid' => $_W['openid'], 'goodsid' => $value['id'], 'optionid' => $value['optionid'], 'marketprice' => $value['marketprice'], 'total' =>$value['total'], 'selected' => 1,'createtime' => time());									
			}
