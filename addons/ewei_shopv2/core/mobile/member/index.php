...
class Index_EweiShopV2Page extends MobileLoginPage 
{
	public function main() 
	{
		...
		$data = m('util')->updateregnum();
		

		
		m('member') -> checklogin($_W['openid']);
		$integral   =$member['credit1'];	
		$smallchange=$member['credit2'];
		if($smallchange>10000){
			$smallchange=number_format(($smallchange/10000),2).'万';
		}
		if($integral>10000){
			$integral=number_format(($integral/10000),2).'万';
		}
	...
	public  function  test(){
		...
		global $_GPC;
		$member = m('member')->getMember($_W['openid'], true);		
		m('member')->levelupdate(5000,$member['levelid']);		
	}
