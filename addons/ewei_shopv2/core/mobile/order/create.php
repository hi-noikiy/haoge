...
class Create_EweiShopV2Page extends MobileLoginPage 
{
	...
	public function main() 
	{
		...
		m('member') -> checklogin($_W['openid']);
		$IsOnce=m('member')->GetIsOnce($_W['openid']);
		$bilist=pdo_getall('ewei_shop_goods',array('isbi'=>1,'deleted'=>'0'));
		if (p('exchange')) 
		{
		...
		//检查是否应该享受折扣Start,员工股东不参与
		/*
		if($member['groupid']<7){
			$isdiscount=false;//是否可享受
			$isdiscountmoney=0;//优惠多少
			$allgoods=$goods_list;
			$tempgoodsmoney=0;//订单中的分享商品价值多少元
			foreach ($allgoods as $key => $value) {
				foreach ($value['goods'] as $value) {
						$oneres=pdo_get('ewei_shop_goods',array('id'=>$value['goodsid']));
						if($oneres['pcate']!=1179 && $oneres['pcate']!=1180){
							$tempgoodsmoney=$tempgoodsmoney+$oneres['marketprice']*$value['total'];
						}				
				}
			}
			$memberdis=m('member')->getMemberDiscount($_W['openid']);//查询会员当前享受的折扣
			$disres=m('order')->getOrderdiscount($memberdis['discountid'],$memberdis['discount'],$tempgoodsmoney);
			$paymoney=$disres['countmoney'];//用户实际支付的分享商品总价格
			$ratemoney=$tempgoodsmoney-$paymoney;//为用户折扣了多少钱
			if($ratemoney>0){
				$isdiscount=true;
				$goodsprice=$goodsprice-$ratemoney;//页面商品价格的变量修改
				$realprice =$realprice-$ratemoney;//页面总价格的变量修改
			}			
		}*/
		//检查是否应该享受折扣End		
	...
	public function getbilist(){
		global $_W;
		global $_GPC;
   		$list= pdo_fetchall('select  id,title,thumb,marketprice from ' . tablename('ewei_shop_goods') . 'where uniacid=:uniacid and deleted=0  and isbi=1  order by id desc',array(':uniacid' => $uniacid));
		show_json(1, $list);		
	}
	...
	public function submit() 
	{
		...
		if ($multiple_order == 0) 
		{
			...
			foreach ($allgoods as $goods ) 
			{
				...
				//折扣的价格在订单中减去Start;股东和员工不参与
				if($member['groupid']<7){
					$tempgoodsmoney=0;//分享商品的真实价格
					$OrderGoods=pdo_getall('ewei_shop_order_goods',array('orderid'=>$orderid));
					foreach ($OrderGoods as $key=>$value) {
						$oneres=pdo_get('ewei_shop_goods',array('id'=>$value['goodsid']));
						$content=$member['realname'].'下单购买'.$oneres['title'].$value['total'].'件';
						m('util') -> insertnotice($content,$content);							
						if($oneres['pcate']!=1179 && $oneres['pcate']!=1180 && $oneres['pcate']!=1181 ){
							$tempgoodsmoney=$tempgoodsmoney+$oneres['marketprice']*$value['total'];
						}
						else{
					...
					$OrderGoods=array_merge($OrderGoods);
					$memberdis=m('member')->getMemberDiscount($_W['openid']);//查询会员当前享受的折扣
					$disres=m('order')->getOrderdiscount($memberdis['discountid'],$memberdis['discount'],$tempgoodsmoney);
					$paymoney=$disres['countmoney'];//用户实际支分享商品总价格
					$ratemoney=$tempgoodsmoney-$paymoney;//为用户折扣了多少钱
					//保证不为负值
					$OneOrder=pdo_get('ewei_shop_order',array('id'=>$orderid));	
					$tomoney=$ratemoney;
					if($tomoney>=$OneOrder['price']){
						$tomoney=$OneOrder['price'];
					}
					//保证不为负值					
					pdo_update('ewei_shop_order',array('discountid'=>$disres['discountid'],'ordergoodsprice'=>$tempgoodsmoney,'ordergoodspayprice'=>$paymoney),array('id'=>$orderid));								
					//折扣的价格在订单中减去End;						
				}
