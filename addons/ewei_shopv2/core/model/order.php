...
class Order_EweiShopV2Model {
	...
	//计算订单中的分享商品价格,豪格币商品价值,会员福利区商品价值
	public  function caculateallorderprice($orders)
	{
		$data=array('share'=>0,'material'=>0,'credit'=>0,'matter'=>0);
		foreach ($orders as $value) {
			$OrderGoods = pdo_fetchall("select a.id as id, a.orderid as orderid,a.goodsid as goodsid,a.price as price,a.total as total,b.pcate as pcate from ".tablename('ewei_shop_order_goods')." as a left join ".tablename('ewei_shop_goods')." as b on a.goodsid=b.id where orderid=".$value['id']);		
			//分享区
			$data['share']+=$value['ordergoodspayprice'];
			foreach ($OrderGoods as $value2) {
				//会员福利区	
				if($value2['pcate']==1179){
					$data['material']+=$value2['price'];
				}
				//豪格币区
				if($value2['pcate']==1180){
					$data['credit']+=$value2['price'];
				}
				//物料区	
				if($value2['pcate']==1181){
					$data['matter']+=$value2['price'];
				}							
			}
			//豪格币去除积分抵扣
			if($value['deductprice']>0){
				$data['credit']-=$value['deductprice'];
			}
		}
	 ...
	 * 计算全平台订单价值
	 * $type=1是上月,2是本月
	 * $from=1是纯分享商品,$from=2是减去分销-返利再乘*92%.最后减掉分红
	 */
	public  function caculateorderprice($type,$from){
		if($type==1){
		...
		$Orders=pdo_fetchall("select *  from ". tablename('ewei_shop_order')." where status=3 and createtime>=".$StartTime." and createtime<=".$EndTime);		
		...
		foreach ($Orders as $value) {
			...
			{
				$TempM+=$value['ordergoodsprice']-($this->getOrderCommission($value['id']))-($value['ordergoodsprice']-$value['ordergoodspayprice']);
			}
		}
		if($from==1){
			return $TempM;	
	...
	public function  clearcart($orderid){
		$ordergoods=pdo_getall('ewei_shop_order_goods',array('orderid'=>$orderid));
		foreach ($ordergoods as $value) {
			pdo_delete('ewei_shop_member_cart',array('goodsid'=>$value['goodsid']));
		}
	}
	...
	//参数1用户的折扣id,参数2是折扣百分比,参数3是订单总金额
	//返回需要支付的实际价格和享受的折扣id
	public function getOrderdiscount($disid = '', $disrate = 0, $money = 0) {
		$data = array('countmoney' => 0, 'discountid' => 0);
		if ($disid == '' && $disrate == 0 && $money == 0) {
			return $data;
		}
		//循环遍历出当前订单价格应该享受的折扣id和折扣比例
		$disall = pdo_fetchall('select id,ordermoney,discount from ' . tablename('ewei_shop_member_discount') . ' where enabled=1 order by ordermoney desc');
		$tempdisid = 0;
		$tempdisrate = 0;
		foreach ($disall as $key => $value) {
			if ($money >= $value['ordermoney']) {
				$tempdisid = $value['id'];
				$tempdisrate = $value['discount'] / 100;
				break;
			}
		}
		//用户默认的折扣比例大于当前订单可以享受的折扣,优先用户的.
		if ($disrate > $tempdisrate) {
			$data['countmoney'] = $money - $money * $disrate;
			$data['discountid'] = $disid;
		} else {
			$data['countmoney'] = $money - $money * $tempdisrate;
			$data['discountid'] = $tempdisid;
		}
		return $data;
	}
