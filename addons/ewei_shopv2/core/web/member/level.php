...
class Level_EweiShopV2Page extends WebPage 
{
	public function main() 
	{
		...
		$others = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_member_discount') . ' WHERE 1 ' . $condition . ' ORDER BY level asc', $params);
	...
	protected function post() 
	{
		...
		{
			$level = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_member_discount') . ' WHERE id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => intval($id)));
			if (!(empty($level))) 
			{
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($id))) 
			{
				...
				else 
				{
					...
					pdo_update('ewei_shop_member_discount', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
			...
			else 
			{
				$data['goodsids'] = $goodsids;
				$data['buygoods'] = $buygoods;
				pdo_insert('ewei_shop_member_discount', $data);
				$id = pdo_insertid();
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,levelname FROM ' . tablename('ewei_shop_member_discount') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		foreach ($items as $item ) 
		{
			pdo_delete('ewei_shop_member_discount', array('id' => $item['id']));
	...
	public function enabled() 
	{
		...
		foreach ($items as $item ) 
		{
			pdo_update('ewei_shop_member_discount', array('enabled' => intval($_GPC['enabled'])), array('id' => $item['id']));
