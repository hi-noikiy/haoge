	...
	public function main()
	{
		...
		if($types!=''){
			foreach ($list as $key=>$value ) {
				if($types==0 && $value['pcate']>1178){
					unset($list[$key]);
				}
				if($types==1 && $value['pcate']!=1179){
					unset($list[$key]);					
				}
				if($types==2 && $value['pcate']!=1180){
					unset($list[$key]);					
				}						
			}				
		}
