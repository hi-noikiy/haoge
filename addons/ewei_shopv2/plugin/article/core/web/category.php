...
class Category_EweiShopV2Page extends PluginWebPage
{
	public function main()
	{
		global $_W;
		global $_GPC;
		$list = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_article_category') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\'  ORDER BY displayorder asc ,id asc');
	...
	public function save()
	{
		...
		if (!empty($_GPC['cate'])) {
			foreach ($_GPC['cate'] as $id => $cate ) {
				$data = array('category_name' => trim($cate['name']), 'uid' => intval($cate['uid']), 'displayorder' => intval($cate['displayorder']), 'isshow' => intval($cate['isshow']));
