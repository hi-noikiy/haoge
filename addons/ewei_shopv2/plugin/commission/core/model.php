...
if (!(class_exists('CommissionModel'))) {
	class CommissionModel extends PluginModel {
		...
		public function allmoney($M){
			...
			$all=pdo_get('ewei_shop_MonthCommission',array('month'=>$month,'openid'=>$_W['openid']));
		...
		public function calculate($orderid = 0, $update = true) {
			...
			$order = pdo_fetch('select  ordergoodsprice,ordergoodspayprice,agentid,price,goodsprice,deductcredit2,discountprice,isdiscountprice,dispatchprice,changeprice,ispackage,packageid from ' . tablename('ewei_shop_order') . ' where id=:id limit 1', array(':id' => $orderid));
			...
			if($member['groupid']<7){
				...
				$commission01 = array('default' => '0');
				$commission02 = array('default' => '0');
				$commission03 = array('default' => '0');
				$commissionall = array('level1' => '0', 'level2' => '0', 'level3' => '0');				
				$OrderGoods = pdo_fetchall("select a.id as id, a.orderid as orderid,a.goodsid as goodsid,a.price as price,a.total as total,b.pcate as pcate from ".tablename('ewei_shop_order_goods')." as a left join ".tablename('ewei_shop_goods')." as b on a.goodsid=b.id where orderid=".$orderid.' and pcate<>1179 and pcate<>1180 and pcate<>1181');		
				//获取用户实际支付的价格
				$money=0;

				if($order['ordergoodsprice']==$order['ordergoodspayprice']){
					$money=$order['ordergoodsprice'];
				}
				else{
					$money=$order['ordergoodspayprice'];
				}
				 
				 if($money>0){
					$counts=count($OrderGoods);
					if ($m1) {
						$M1Price = sprintf("%.2f", m('member') -> CalculateDiscount(1,$money)*$money);
						$M1Price=$M1Price/$counts;
						$commission01['default'] = (string)$M1Price;
						$commissionall['level1'] = (string)$M1Price;				
					}
					if ($m2) {
						$M2Price = sprintf("%.2f", m('member') -> CalculateDiscount(2,$money)*$money);
						$M2Price=$M2Price/$counts;
						$commission02['default'] = (string)$M2Price;
						$commissionall['level2'] = (string)$M2Price;										
					}			
					foreach ($OrderGoods as $key => $value) {
						pdo_update('ewei_shop_order_goods', array('commission1' => serialize($commission01), 'commission2' => serialize($commission02), 'commission3' => serialize($commission03), 'commissions' => serialize($commissionall)), array('id' =>$value['id']));					
					}				 	
				 }
